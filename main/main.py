...
class VoiceMoodTreeHole:
    ...
    def load_models(cls, force_reload=False):
        ...
        try:
            ...
            # 初始化大语言模型
            print("正在加载大语言模型...")
            cls._language_model = LanguageModel(
                use_deepseek=False)  # 使用Qwen_1.8B模型
    ...
    def predict_emotion(self, audio_path):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # 如果有GPU，将输入移至GPU
                if torch.cuda.is_available():
                    inputs = {k: v.to(VoiceMoodTreeHole._device)
            ...
            # 预测 - 在GPU上运行
            with torch.no_grad():
                emotion_output, intensity_output = VoiceMoodTreeHole._emotion_model(
                    features)
                ...
                # 解码预测结果
                emotion_code = VoiceMoodTreeHole._emotion_encoder.inverse_transform(
                    emotion_pred.cpu().numpy())[0]
                intensity_code = VoiceMoodTreeHole._intensity_encoder.inverse_transform(
                    intensity_pred.cpu().numpy())[0]
    ...
    def process_audio_json(self, audio_path):
        """
        处理音频文件并生成回应
        """
        ...
        output_file = os.path.join(
            self.output_dir, f"{os.path.basename(audio_path).split('.')[0]}_result.json")
