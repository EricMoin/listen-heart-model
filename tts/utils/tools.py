...
def synth_samples(batch, outputs, vocoder, model_config, preprocess_config, path):
    """
    合成音频样本
    """
    # 获取模型输出 
    mel_outputs = outputs[1].transpose(1, 2).cpu().numpy()
    mel_lengths = outputs[2].cpu().numpy()
    
    # 打印形状以便调试
    print(f"mel_outputs shape: {mel_outputs.shape}")
    print(f"mel_lengths shape: {mel_lengths.shape}")
    ...
    try:
        # 将mel输出转换为tensor
        mel_tensor = torch.FloatTensor(mel_outputs).to(device)
        
        # 使用vocoder_infer函数生成音频
        wavs = vocoder_infer(mel_tensor, vocoder, model_config, preprocess_config)
        
        # 保存每个样本
        for i, wav in enumerate(wavs):
            # 检查wav是否为None
            if wav is None:
                print(f"样本 {i} 的生成结果为None，跳过")
                continue
                
            # 保存音频文件
            output_path = os.path.join(path, f"{ids[i]}.wav")
            
            # 如果不需要重采样，直接保存
            sampling_rate = preprocess_config["preprocessing"]["audio"]["sampling_rate"]
            sf.write(output_path, wav, sampling_rate)
            print(f"样本 {i} 已保存到 {output_path}")
        ...
        for i, mel_output in enumerate(mel_outputs):
            try:
                # 获取当前样本的长度 - 处理可能的多维数组
                if mel_lengths.ndim > 1:
                    mel_length = int(mel_lengths[i][0])
                else:
                    mel_length = int(mel_lengths[i])
                    
                print(f"样本 {i}, mel_length: {mel_length}")
                
                # 确保mel_output有正确的形状
                mel = mel_output[:mel_length]
                ...
                # 使用vocoder_infer函数生成音频
                wav = vocoder_infer(mel_tensor, vocoder, model_config, preprocess_config)[0]
